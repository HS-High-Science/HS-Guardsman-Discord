...
export default class PullCommand implements ICommand
{
    ...
    async execute(interaction: ChatInputCommandInteraction<"cached">): Promise<void>
    {
        ...
        exec(`git pull https://AstroHWeston:glpat-AxgPPWPp8SBXtsPcCMmv@gitlab.astrohweston.xyz/high-science/guardsman-discord.git ${interaction.options.getString("branch")}`, async (error, stdout, stderr) =>
        {
            if (error)
            {
